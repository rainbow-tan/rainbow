...
from MyDatabase import update_database
import Select
...
def update_html(number_id=''):
    def update():
        ...
        update_database(db, 'employee', 'number', number, insert_data)
        fanhui()
    ...
    def get_info_by_db():
        if number_id:
            db_data = select_database(db, 'employee', ['number', 'name', 'sex', 'hiredate', 'position', 'pay'],
                                      {'number': number_id})
            if len(db_data) >= 1:
                number_entry.insert(END, db_data[0]['number'])
                number_entry.config(state='readonly')
                name_entry.insert(END, db_data[0]['name'])
                sex_value.set(db_data[0]['sex'])
                intodate_entry.insert(END, db_data[0]['hiredate'])
                job_entry.insert(END, db_data[0]['position'])
                pay_entry.insert(END, db_data[0]['pay'])
    ...
    frame2 = Frame(win)
    frame1.pack(pady=10)
    Label(text='员工信息更新', font=('粗体', 26), fg='green').pack()
