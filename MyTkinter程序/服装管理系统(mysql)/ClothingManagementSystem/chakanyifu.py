...
def update():
    ...
    if len(selects) != 1:
        ...
        print(choose_data)
        win.destroy()
        os.system('python update_yifu.py {}'.format(choose_data[0]))
...
def delete():
    ...
    if len(selects) > 0:
        ...
        if choose:
            for i in remove_ids:
                sql = 'delete from yifu where mingcheng="{}"'.format(i)
                print(sql)
                cursor = db.cursor()
                try:
                    # 执行SQL语句
...
if __name__ == '__main__':
    ...
    field_names = ['服装名称', '服装价格', '适用季节', '来源厂家', '导购职工', '存放仓库']

    data = shujuku.select_all(db, 'yifu')
    print(data)
    field_data_value = [['服装名称', '服装价格', '适用季节', '来源厂家', '导购职工', '存放仓库'],
                        ['服装名称', '服装价格', '适用季节', '来源厂家', '导购职工', '存放仓库']]
