...
def update():
    ...
    if len(selects) != 1:
        ...
        print(choose_data)
        win.destroy()
        os.system('python update_changjia.py {}'.format(choose_data[0]))
...
def delete():
    ...
    if len(selects) > 0:
        ...
        if choose:
            for i in remove_ids:
                sql = 'delete from changjia where bianhao="{}"'.format(i)
                print(sql)
                cursor = db.cursor()
                try:
                    # 执行SQL语句
                    ...
                    # 发生错误时回滚
                    messagebox.showerror('提示信息', '删除失败:{}'.format(e))
                    db.rollback()
            win.destroy()
...
if __name__ == '__main__':
    ...
    field_names = ['厂家编号', '厂家名称', '厂家性质', '厂家地址', '联 系 人', '联系方式']
    data = shujuku.select_all(shujuku.db, 'changjia')
    field_data_value = [['厂家编号', '厂家名称', '厂家性质', '厂家地址', '联 系 人', '联系方式'],
                        ['厂家编号', '厂家名称', '厂家性质', '厂家地址', '联 系 人', '联系方式']]
    field_data_value = data
    frame = tkinter.Frame(win)
    select_label = Label(win, text='查看信息', font=('', 20))
