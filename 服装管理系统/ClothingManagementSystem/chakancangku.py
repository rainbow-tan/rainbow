...
def update():
    ...
    if len(selects) != 1:
        ...
        print(choose_data)
        win.destroy()
        os.system('python update_cangku.py {}'.format(choose_data[0]))
        pass
...
def delete():
    ...
    if len(selects) > 0:
        ...
        if choose:
            for i in remove_ids:
                sql = 'delete from cangku where bianhao="{}"'.format(i)
                print(sql)
                cursor = db.cursor()
                try:
                    # 执行SQL语句
...
if __name__ == '__main__':
    ...
    field_names = ['仓库编号', '仓库名称', '仓库地址', '建成日期', '管理员', '联系方式', ]

    data = shujuku.select_all(shujuku.db, 'cangku')
    # field_data_value = [['仓库编号', '仓库名称', '仓库地址', '建成日期', '管理员', '联系方式', ],
