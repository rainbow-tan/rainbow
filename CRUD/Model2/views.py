...
def select_html(request):
    ...
    database.paging(sort_data, pagesize, current_page, data_dict)  # 分页
    database.export(sort_data, data_dict, u'自闭档案导出模板.xls', '2')  # 导出

    return render(request, 'Model2/select.html', data_dict)
...
def add(request):
    ...
    return redirect("/Model2/select.html/")
...
def import_html(request):
    return render(request, 'Model2/import.html')
...
def import_data(request):  # 导入
    ...
    return render(request, 'Model2/import.html', data_dict)
