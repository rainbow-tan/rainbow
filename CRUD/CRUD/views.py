...
def login(request):
    ...
    if username == 'nanbowan' and password == 'nanbowan':
        response = redirect("/index.html/")
        response.set_cookie("username", username)
        response.set_cookie("is_login", True)
        response.set_cookie("can_delete", True)
        response.set_cookie("show_add", True)
    elif username == 'admin' and password == 'admin':
        response = redirect("/index.html/")
        response.set_cookie("username", username)
        response.set_cookie("is_login", True)
        response.set_cookie("can_delete", True)
        response.set_cookie("show_add", False)
    else:
...
def init_db():
    ...
    qujianmingchen = ['京哈中继13', '京哈中继14', '京哈中继15', '京哈中继16', '京哈牛河梁', '京哈中继17', '孙家沟线路所', '喀左',
                      '京哈中继18', '京哈中继19', '奈林皋', '京哈中继20', '京哈中继21', '朝阳', '京哈中继22', '京哈中继23', '北票',
                      '京哈中继24', '京哈中继25', '京哈中继26', '乌兰木图', '京哈中继27', '京哈中继28', '阜新', '京哈中继29',
                      '京哈中继30', '京哈中继31', '黑山北', '京哈中继32', '京哈中继33', '姚家窝铺线路所', '新民北', '沈阳西',
                      '京哈中继34', '京哈中继35', '田家窝铺线路所', '新通中继1', '新通中继2', '彰武', '新通中继3', '章古台',
                      '新通中继4', '甘旗卡', '新通中继5', '新通中继6', '新通中继7', '木里图', '通辽高速场', '沈大中继1', '沈大中继2',
                      '普湾', '沈大中继3', '沈大中继4', '瓦房店西', '沈大中继5', '沈大中继6', '沈大中继7', '沈大中继8', '鲅鱼圈',
                      '沈大中继9#', '盖州西', '沈大中继10', '营口东', '沈大中继11', '下夹河线路所', '海城西', '沈大中继12',
                      '沈大中继13', '鞍山西', '沈大中继14', '盘营中继1', '盘锦高速场', '盘营中继2', '盘营中继3', '盘营中继4',
                      '中小线路所', '辽阳高速场', '沈大中继15', '沈大中继16', '沈大中继17', '沈阳南', '沙河堡线路所', '沈丹中继1',
                      '本溪新城', '沈丹中继2', '沈丹中继3', '南芬北', '本溪客车场', '沈丹中继4', '通远堡西', '沈丹中继5', '凤城东',
                      '沈丹中继6', '五龙背东', '沈南动车所', '沈北动车所', '通辽动车所', '郑仗子线路所', '建平', '喀赤中继1', '喀赤中继2',
                      '喀赤宁城', '喀赤中继3', '喀赤中继4', '平庄', '喀赤中继5', '喀赤中继6', '赤峰高速场', ]
    ...
    fangzhuangqiang = ['有防撞墙', '无防撞强', '无数据', '低于轨面', '低于25mm',
                       ]
    for i in fangzhuangqiang:
        Field26Table(field1=i.strip()).save()
    shebenleixing = ['进站信号机', '出站信号机', '调车信号机', 'ZDJ9型道岔', 'ZD6型道岔', '扼流变压器', '电热融雪箱', '电缆盒',
                     '绝缘破损防护TB', '调谐匹配单元', '空心线圈', '信号标志牌', '器材XB箱', '其它', 'LEU箱', '无数据',
                     ]
    for i in shebenleixing:
        Field27Table(field1=i.strip()).save()
    biaochenzaiping = ['1700-1', '2000-1', '2300-1', '2600-1', '1700-2', '2000-2', '2300-2',
                       '2600-2']
