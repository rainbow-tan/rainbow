...
from CRUD.Pub import CURRENT_PAGE, PAGESIZE
...
from Model4 import Business, DB
...
def select_html(request):
    ...
    paginator = Paginator(sort_filter_data, pagesize)
    ...
    folder = '/four/data_{}.xls'.format(Pub.get_time())
    ...
    source_file = os.path.join(Pub.EXPORT_TEMPLATE_FOLDER, u'移频轨道电路测试导出模板.xls')
    Pub.append_xls(source_file, save_path, write_data)

    return render(request, 'four/select.html', data_dict)
...
def add(request):
    ...
    return redirect("/four/select.html/")
...
def import_html(request):
    return render(request, 'four/import.html')
...
def import_data(request):  # 导入
    ...
    return render(request, 'four/import.html', data_dict)
