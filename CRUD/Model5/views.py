...
from Model5 import XLS
from Model5 import bs
from Model5 import db
from Model5.bs import qiuhe
...
def select_html(request):
    ...
    database.export(all_data, data_dict, u'录入页导出模板.xls', '5')  # 导出
    return render(request, 'Model5/select.html', data_dict)
...
def add_html(request):
    ...
    data_dict['xianbies'] = Pub.select_column(XianbieChejianChenzhan, 'xianbie')
    data_dict['xiangmus'] = Pub.select_column(ZhuYe, 'xiangmu')
    return render(request, 'Model5/add.html', data_dict)
...
def add(request):
    ...
    return redirect('/Model5/zhuye.html/')
...
def update_html(request):
    ...
    return render(request, 'Model5/update.html', data_dict)
...
def update(request):
    ...
    return redirect("/Model5/select.html/")
...
def zhuye_html(request):
    ...
    data_dict['xiangmus'] = Pub.select_column(ZhuYe, 'xiangmu')
    data_dict['fenleis'] = Pub.select_column(ZhuYe, 'fenlei')
    return render(request, 'Model5/zhuye.html', data_dict)
...
def zhuye(request):
    ...
    yuefen = request.GET.get('yuefen', '')
    chezhans = []
    qianduan_chejians = bs.split_string(qianduan_chejians)
    ...
    for xiangmu in xiangmus:
        ...
        for obj_by_xiangmu in obj_by_xiangmus:
            fenleis += bs.set_fenleis(yuefen, obj_by_xiangmu, qianduan_fenlei)
        ...
        for fenlei in fenleis:
            ...
            zong_benyue_history = '/Model5/history.html/?xiangmu={}&fenlei={}&chadangyue=True'.format(
                    xiangmu, fenlei)
            zong_leiji_history = '/Model5/history.html/?xiangmu={}&fenlei={}'.format(xiangmu,
                                                                                     fenlei)
            for item in all_chejian_chezhans:
                ...
                for chezhan in chezhan_names:
                    ...
                    if zhuye_obj:
                        ...
                        url_benyuye = '/Model5/history.html/?chejian={}&chezhan={}&xiangmu={}&fenlei={}&chadangyue=True'.format(
                                chejian_name,
                                chezhan,
                                common_obj.xiangmu,
                                fenlei)
                        ...
                        url_leiji = '/Model5/history.html/?chejian={}&chezhan={}&xiangmu={}&fenlei={}'.format(
                                chejian_name,
                                chezhan,
                                common_obj.xiangmu,
                                fenlei)
            ...
            index += 1
        data_dict['body'] = database_data
    folder = 'Model5/export_{}.xls'.format(Pub.get_time())
...
def history_html(request):
    ...
    return render(request, 'Model5/history.html', data_dict)
...
def history_add_html(request):
    ...
    return render(request, 'Model5/history_add.html', data_dict)


def import_html(request):
    return render(request, 'Model5/import.html')
...
def see_data(request):
    ...
    if 'file' in request.FILES:
        ...
        filename = os.path.join(Pub.IMPORT_FOLDER,
                                'data_Model5_check_{}.xls'.format(Pub.get_time()))
    ...
    return render(request, 'Model5/import.html', data_dict)
...
def import_data(request):
    ...
    if 'file' in request.FILES:
        file = request.FILES['file']
        filename = os.path.join(Pub.IMPORT_FOLDER, 'Model5', 'import_{}.xls'.format(Pub.get_time()))
        Pub.save_file(filename, file)
        try:
            file_data = Pub.read_xls(filename, 2)
