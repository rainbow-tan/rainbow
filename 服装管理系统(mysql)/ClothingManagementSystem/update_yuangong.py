...
def add():
    ...
    try:
        ...
        messagebox.showwarning('提示信息', '薪资填写错误')
        return
    cursor = db.cursor()
    bianhao = shujuku.select_one(db, 'yuangong', 'bianhao')
    xingming = shujuku.select_one(db, 'yuangong', 'xingming')
    print(bianhao)
    ...
    # SQL 更新语句
    sql = 'UPDATE yuangong SET xingming="{}",xingbie="{}",zhiwei="{}",xingzi="{}",lianxifangshi="{}" WHERE bianhao="{}"'.format(
    ...
    # SQL 更新语句
    sql = 'UPDATE yifu SET yuangong="{}" WHERE yuangong="{}"'.format(value2, old_name)
    print(sql)
    try:
        # 执行SQL语句
        cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2:
        ...
        sql = 'select * from yuangong where bianhao="{}"'.format(choose_id)
        cursor = db.cursor()
        print(sql)
        cursor.execute(sql)
        # 获取所有记录列表
        ...
        data = ['员工编号', '员工姓名', '员工性别', '员工职位', '员工薪资', '联系方式', ]

    f = Frame(win)
    Label(f, text='员工编号', font=('', 20)).grid(row=0, column=0, padx=30, pady=15)
    Label(f, text='员工姓名', font=('', 20)).grid(row=1, column=0, padx=30, pady=15)
    Label(f, text='员工性别', font=('', 20)).grid(row=2, column=0, padx=30, pady=15)
    Label(f, text='员工职位', font=('', 20)).grid(row=3, column=0, padx=30, pady=15)
    Label(f, text='员工薪资', font=('', 20)).grid(row=4, column=0, padx=30, pady=15)
    ...
    Button(f, text='员工', font=('', 20), relief='g', command=fanhui).grid(row=7, column=1)
